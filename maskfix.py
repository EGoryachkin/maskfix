...
def main():
	...
	while img_index < len(imglist) and img_index >= 0:
		...
		if os.path.exists(imglist[img_index][0:-3]+'xml'):
			...
			tmp.text = os.path.basename(imglist[img_index])
			tmp = ET.SubElement(root, 'size')

			tmp2 = ET.SubElement(tmp, 'width')
			tmp2.text = str(w)
			tmp2 = ET.SubElement(tmp, 'height')
			tmp2.text = str(h)
			tmp2 = ET.SubElement(tmp, 'depth')
			tmp2.text = str(d)
			...
			if(object_index >= 0):
				...
				while 1:
					...
					if   key == ord('1'):
						...
						cv.floodFill(mask, tmp_mask, (x_,y_), (255,255,255));
